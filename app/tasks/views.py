...
def task_view(request, task_id):
    try:
        ...
        logger.warning(f'User "{request.user}" tried to access task that does not exist.')
        messages.warning(request, 'Task does not exist. Redirected back to tasks.')
...
def task_terminate(request, task_id):
    if request.method == 'POST':
        ...
        if task.task_result.status == 'PENDING':
            ...
            messages.info(request, f'{task_info["task_name"]} task revoked ({task_id}).')
        elif task.task_result.status == 'PROGRESS':
            celery_app.control.revoke(task_id, terminate=True)
            messages.info(request, f'{task_info["task_name"]} task terminated ({task_id}).')
        else:
            messages.warning(request, f"Couldn't terminate task. Task was already completed.")
        return redirect('task-list')
    task = Task.objects.get(task_id=task_id)
    return render(request, 'tasks/task_terminate.html', {
        'task_id': task_id})

@login_required
@user_passes_test(user_is_data_admin)
def task_list(request):
    tasks = Task.objects.select_related('task_result').all().order_by('-task_result__date_created')
    if not tasks:
    ...
    task_info = dict(zip(
        task_ids, 
        list(map(lambda x: json.loads(x), task_info_list))))
    results_list = TaskResult.objects.filter(task_id__in=task_ids)
    task_results = dict(zip(
