...
def create_master_data(self, source_id):
    ...
    with transaction.atomic():
        ...
        Value.objects.bulk_create(new_value_objs, batch_size=5000)
        logger.info(f"Saving master_entities to database "
                    f"(object count {len(new_master_entity_objs)})")
        MasterEntity.objects.bulk_create(new_master_entity_objs, 
                                         batch_size=5000)
...
def delete_master(self, source_id):
    ...
    with transaction.atomic():
        (MasterData.objects
                .filter(source_datas__source_entity__source_id=source_id)
                .delete())
        (Value.objects.filter(master_datas__isnull=True)
                      ...
                      .delete())
        (MasterSourceData.objects
                .filter(source_data__source_entity__source_id=source_id)
                .delete())
