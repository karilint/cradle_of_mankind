...
@remember_last_query_params(
    "quality-control-list", ["page", "workflow", "status"]
)
def quality_control_list(request):
    ...
    return render(
        request,
        "quality_control/quality_control_list.html",
        {
            "workflows": workflows,
...
def quality_control_check(request, workflow_pk, scan_pk):
    ...
    if request.method == "POST":
        ...
        for question in tasks.values():
            ...
            if answer:
                ...
                final.save()
        return redirect(
            "quality-control-check", scan_pk=scan.id, workflow_pk=workflow.id
        )
    ...
    return render(
        request,
        "quality_control/quality_control_check.html",
        {
            "current_workflow": workflow,
            "scan": scan,
            "prev_scan": prev_scan,
...
def summary_list(request):
    ...
    return render(
        request,
        "quality_control/summary_list.html",
        {"workflows": workflows, "statuses": statuses, "page_obj": scans},
    )
...
def summary_check(request, scan_pk):
    ...
    return render(
        request,
        "quality_control/summary_check.html",
        {
            "scan": scan,
            "prev_scan": prev_scan,
            "next_scan": next_scan,
...
def get_next_and_prev_scans(request, scan):
    ...
    retirement_list = retirement_list.filter(status=status)
    prev_item = (
        retirement_list.filter(subject__scan__id__lt=scan.id)
        .order_by("-subject__scan__id")[:1]
        .first()
    )
    next_item = (
        retirement_list.filter(subject__scan__id__gt=scan.id)
        .order_by("subject__scan__id")[:1]
        .first()
    )
    ...
    for retirement in retirements:
        ...
        for task in tasks:
            ...
            for annotation in annotations:
                answer_counts[annotation.value] = (
                    answer_counts.get(annotation.value, 0) + 1
                )
            if answer_counts:
...
def get_questions_and_values(retirement, tasks):
    ...
    for task, name in tasks.items():
        ...
        annotations = []
        for classification in classifications:
            annotations.extend(classification.annotation_set.filter(task=task))
        answers = {}
        for annotation in annotations:
            answers[annotation.value] = answers.get(annotation.value, 0) + 1
        answers = {
            k: v
            for k, v in sorted(
