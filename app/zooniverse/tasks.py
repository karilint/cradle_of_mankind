...
def update_zooniverse_data(self):
    ...
    classifications_without_retirement = set(
        Classification.objects.filter(retirement=None).values_list(
            "id", flat=True
        )
    )
    existing_annotations = set(
        map(
            ...
            Annotation.objects.all().values_list("classification_id", "task"),
        )
    )
    new_workflows = []
    new_subjects = []
    ...
    record_progress(
        ...
        total_work,
        1,
        f"Saving zooniverse data to database... This might take awhile...",
    )
    with transaction.atomic():
        ...
        record_progress(
            ...
            total_work,
            1,
            f"Saving workflows (1/5) to database...",
        )
        ...
        record_progress(
            ...
            total_work,
            1,
            f"Saving subjects (2/5) to database...",
        )
        ...
        record_progress(
            ...
            total_work,
            1,
            f"Saving retirements (3/5) to database...",
        )
        ...
        record_progress(
            ...
            total_work,
            1,
            f"Saving classifications (4/5) to database...",
        )
        Classification.objects.bulk_create(
            new_classifications, batch_size=5000
        )
        Classification.objects.bulk_update(
            classifications_to_be_updated, ["retirement"], batch_size=5000
        )
        ...
        record_progress(
            ...
            total_work,
            1,
            f"Saving annotations (5/5) to database...",
        )
...
def save_classification(
    ...
    if classification_id not in existing_classification_ids:
        ...
        new_classification.created_at = eet.localize(
        ...
        classification_id in classifications_without_retirement
        and retirement_id
    ):
        ...
        for _, subject_row in subject_data.items():
            if subject_row["retired"]:
                ...
                if retirement_id not in existing_retirement_ids:
                    ...
                    new_retirement.classifications_count = retirement[
                        "classifications_count"
                    ]
                    new_retirement.created_at = eet.localize(
                        datetime.strptime(
                            retirement["created_at"][:19], "%Y-%m-%dT%H:%M:%S"
                        )
                    )
                    new_retirement.updated_at = eet.localize(
                        datetime.strptime(
                            retirement["updated_at"][:19], "%Y-%m-%dT%H:%M:%S"
                        )
                    )
                    new_retirement.retired_at = eet.localize(
                        datetime.strptime(
                            retirement["retired_at"][:19], "%Y-%m-%dT%H:%M:%S"
                        )
                    )
                    new_retirement.retirement_reason = retirement[
                        "retirement_reason"
                    ]
